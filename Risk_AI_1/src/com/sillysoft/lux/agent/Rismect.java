...
//	 		Take over as much land as possible. 
	...
	public void attackPhase()
		{
			RismectNode root = new RismectNode(this.ID, board);
			
			long start = System.currentTimeMillis();
			
			while( start > System.currentTimeMillis() - 5000 ){
				
				RismectNode selectedNode = treePolicy(root);
			...
			ListIterator<RismectMove> moveSet = root.getFinalMoves().listIterator();
			RismectMove cur;
			...
			while(moveSet.hasNext()){
				...
				if(cur.us.player == this.ID)
					...
					case RismectMove.ATTACK : {
							assert(cur.allout);
							board.attack(cur.us.code, cur.attacking.code, cur.allout);
	...
	private int defaultPolicy(RismectState currentState) {
		
		while( ! currentState.isTerminal()){
		
			RismectMove move = null;
			try {
	...
	private RismectNode treePolicy(RismectNode current) {
